...
from avp_stream import VisionProStreamer  # noqa: F401  (import kept; worker imports again)
from beingbeyond_d1_sdk.pin_kinematics import D1Kinematics, D1KinematicsConfig  # noqa: E402
...
@dataclass(frozen=True)
class AssetCfg:
    asset_root: str = os.path.join(os.path.dirname(os.path.abspath(__file__)), "being_arm_feetech_v2")
    asset_file: str = "robot_with_o6.urdf"
    ...
    def urdf_path(self) -> str:
        return os.path.join(self.asset_root, self.asset_file)
...
def streamer_worker(ip: str, stop_event, shm_cfg: SharedMemCfg, rgb_cfg: RgbShmCfg, runtime_cfg: RuntimeCfg) -> None:
    ...
    """
    from avp_stream import VisionProStreamer  # local import in subprocess
    ...
    while not stop_event.is_set():
        try:
            ...
            # Write control SHM when new raw object arrives
            ...
            # Feed video frames from RGB SHM back to streamer
            ...
            if time.time() - last_log_time > 1.0:
                val_readback = float(shared_buffer[6])  # wrist[0,3] in flattened 4x4
                video_idx_log = f"{last_rgb_frame_idx:.0f}" if runtime_cfg.visualize_video else "Off"
                print(
                    f"[StreamerProcess] Frames: {frame_count} | SHM Wrist[0,3] (Tx): {val_readback:.4f} | Video Idx: {video_idx_log}"
                )
                last_log_time = time.time()
...
def create_sim(gym, args, cfg: SimCfg) -> gymapi.Sim:
    ...
    # Keep forced CPU pipeline behavior
...
def load_asset(gym, sim, cfg: AssetCfg) -> gymapi.Asset:
    ...
    print(f"Loading asset '{cfg.asset_file}' from '{cfg.asset_root}'")
...
def configure_dofs(gym, env, actor, asset, cfg: DofDriveCfg) -> Tuple[int, np.ndarray]:
    # Use asset directly (more compatible than get_actor_asset)
...
def main() -> None:
    ...
    if args.ip:
        ...
        # Cleanup stale SHM from previous runs
    ...
    # Optional camera sensor for video streaming
    camera_handle = None
    ...
    # Kinematics / Retargeting
    # -----------------------------
    print(f"Initializing D1 Kinematics with URDF: {asset_cfg.urdf_path}")
    kin = init_kinematics(asset_cfg.urdf_path)
    ...
    sim_frame_idx = 0  # monotonic (used for RGB SHM counter)
    ...
    while not gym.query_viewer_has_closed(viewer):
        ...
        # Read latest control SHM (if available)
        has_hand_data = False
        curr_pos = None
        curr_R = None
        fingers_mat = None
        ...
        # Push IsaacGym camera image to RGB SHM (if enabled)
        if runtime_cfg.visualize_video and rgb_shm is not None and camera_handle is not None and rgb_counter is not None and rgb_data is not None:
            gym.render_all_camera_sensors(sim)
            image = gym.get_camera_image(sim, env, camera_handle, gymapi.IMAGE_COLOR)
            image = image.reshape(rgb_cfg.height, rgb_cfg.width, 4)  # RGBA
            rgb_data[:] = image[:, :, :3]  # RGB
            rgb_counter[0] = float(sim_frame_idx)  # monotonic counter for streamer
