...
class RobotHandDatasetSAPIENViewer(HandDatasetSAPIENViewer):
    ...
    def render_dexycb_data(self, data: Dict, fps=5, y_offset=0.8):
        ...
        for i in trange(start_frame, num_frame):
            ...
            for robot, retargeting, retarget2sapien in zip(self.robots, self.retargetings, self.retarget2sapien):
                ...
                ref_value = prepare_position_retargeting(joint, indices)
                #ref_value = prepare_vector_retargeting(joint, indices)
    ...
    # 7.1: render poses retargeted from realtime videos
    def render_realtime_data(self, queue, data, y_offset=0.8):
        # Set table and viewer pose for better visual effect only
        global_y_offset = -y_offset * len(self.robots) / 2
        self.table.set_pose(sapien.Pose([0.5, global_y_offset + 0.2, 0]))
        if not self.headless:
        ...
        while True:
            try:
                joint_pos, wrist_xyz, wrist_rpy = queue.get(timeout=1)
            except Empty:
            ...
            #hand_pose_frame = hand_pose[i]
            #vertex, joint = self._compute_hand_geometry(hand_pose_frame)
            #print(hand_pose_frame.shape, joint.shape)
            #print(joint[0])
            joint = joint_pos
            ...
            # Update poses for robot hands
            for robot_name, robot, retargeting, retarget2sapien in zip(self.robot_names, 
                ...
                ref_value = joint[task_indices, :] - joint[origin_indices, :]
                qpos = retargeting.retarget(ref_value)[retarget2sapien]
                #print(robot_name, qpos.shape)
                #if robot_name==RobotName.inspire:
                #    print(qpos)
                qpos[0:3] = wrist_xyz
                qpos[3:6] = wrist_rpy
                robot.set_qpos(qpos)
